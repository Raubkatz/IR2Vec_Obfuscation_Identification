"""
ExtraTrees FINAL Cascading Evaluation (Exp0 test → Exp1→Exp2→(Exp3|Exp4)).

Flow:
  - Load Exp0 test set as the evaluation pool.
  - Stage 1: Exp1 model predicts ObfBinary ∈ {Non_Obfuscated, Obfuscated}.
      * If Non_Obfuscated → final label = the exact non-obf token present in Exp0 test set.
...
Models (auto-picked unless overridden):
  ./Results_ExtraTrees_Classifier/experiment_1/models/BestModel_*.pkl + BestScaler_*.pkl
  ./Results_ExtraTrees_Classifier/experiment_2/models/BestModel_*.pkl + BestScaler_*.pkl
  ./Results_ExtraTrees_Classifier/experiment_3/models/BestModel_*.pkl + BestScaler_*.pkl
  ./Results_ExtraTrees_Classifier/experiment_4/models/BestModel_*.pkl + BestScaler_*.pkl

Features:
  Prefer "0".."299"; else use all numeric cols except the target.

Outputs:
  ./Results_ExtraTrees_Classifier/experiment_CASCADING/confusion_matrices/*.png|*.eps
  ./Results_ExtraTrees_Classifier/experiment_CASCADING/result_txts/*_classification_report.txt
  ./Results_ExtraTrees_Classifier/experiment_CASCADING/result_txts/*_latex_classification_report.txt
  ./Results_ExtraTrees_Classifier/experiment_CASCADING/result_txts/*_run_info.txt
"""
...
RESULTS_ROOT = "./Results_ExtraTrees_Classifier/experiment_CASCADING"
MODELS_ROOT  = "./Results_ExtraTrees_Classifier"
...
def load_model_and_scaler_for_experiment(exp_nr: int):
    """Auto-pick newest BestModel_*.pkl + BestScaler_*.pkl for an experiment."""
    mdir = os.path.join(MODELS_ROOT, f"experiment_{exp_nr}", "models")
    mdl = newest_file(os.path.join(mdir, "BestModel_*.pkl"))
    ...
    if not isinstance(model, ExtraTreesClassifier):
        raise TypeError(f"[Exp {exp_nr}] Model is not an ExtraTreesClassifier.")
...
print("\n[INFO] FINAL Cascading Classification Report:")
print(report)
...
with open(clsrep_path, "w", encoding="utf-8") as f:
    f.write("FINAL Cascading Classification Report\n")
    f.write(report + "\n")
...
latex_lines.append(r"\caption{Cascading Evaluation: Accuracy = " + f"{acc:.4f}" + r"}")
latex_lines.append(r"\label{tab:cascading_report}")
