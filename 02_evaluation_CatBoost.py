...
# -------------------------------------
MODEL_NAME = "BestModel_Obfus_20250306154011.cbm.pkl"  # Example: rename as needed
SCALER_NAME = "BestScaler_Obfus_20250306154011.pkl"  # Example: rename as needed

TEST_DATA_PATH = "./DATA/test_obfuscation_data_programs_cleaned_06032025.csv"
...
# We will chunk the CLASSES, not the features
CLASS_CHUNK_SIZE = 7  # Number of classes per subset for confusion matrices

# Output directories for results
CONF_MATRIX_DIR = "./Results_CatBoost_Classifier/confusion_matrices"
RESULT_TXT_DIR = "./Results_CatBoost_Classifier/result_txts"
...
# -------------------------------------
model_path = f"./Results_CatBoost_Classifier/models/{MODEL_NAME}"
scaler_path = f"./Results_CatBoost_Classifier/models/{SCALER_NAME}"
...
# -------------------------------------
# 3. Single-Run (All Classes)
# -------------------------------------
y_pred = model.predict(X_test_scaled)

report_single_run = classification_report(y_test, y_pred)
...
chunk_conf_matrices = []  # Will hold (chunk_classes, conf_matrix_chunk) for the next step
...
for i in range(num_class_chunks):
    ...
    # Filter the test set to ONLY rows whose true label is in chunk_classes
    chunk_mask = y_test.isin(chunk_classes)
    X_chunk = X_test_scaled[chunk_mask]
    y_chunk = y_test[chunk_mask]

    if X_chunk.shape[0] == 0:
        print(f"\n[INFO] Chunk {i + 1}: No samples belong to classes {chunk_classes}. Skipping.")
        continue

    y_pred_chunk = model.predict(X_chunk)

    # Ignore any predictions that are NOT in chunk_classes
    ...
    if len(y_chunk_filtered) == 0:
        print(f"[INFO] Chunk {i + 1}: All predictions fell outside the chunk classes. Skipping.")
        continue
    ...
    print(f"\n[INFO] Chunk {i + 1}/{num_class_chunks} | Class range: {chunk_classes}")
    print(
        f"Number of samples in chunk: {len(y_chunk)} (kept {len(y_chunk_filtered)} after ignoring predictions outside chunk)")
...
for key, val in report_dict.items():
    # Skip 'accuracy' since it's a single float; will handle separately
