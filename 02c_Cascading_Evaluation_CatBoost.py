"""
CatBoost evaluation for cascaded experiments (0..5).
...
Outputs:
    ...
    ./Results_CatBoost_Classifier/experiment_{exp_nr}/confusion_matrices
    ./Results_CatBoost_Classifier/experiment_{exp_nr}/result_txts
"""
...
MODEL_NAME = None           # e.g., "BestModel_Exp0_20250306154011.cbm" or ".cbm.pkl" (optional override)
...
def resolve_artifacts(models_dir, model_name, scaler_name):
    """
    Resolve model (prefer native .cbm, else .cbm.pkl) and scaler (.pkl).
    If names are None, pick newest matching files.
    """
    # Model
    if model_name is None:
        mdl = newest_file(os.path.join(models_dir, "BestModel_*.cbm"))
        if mdl is None:
            mdl = newest_file(os.path.join(models_dir, "BestModel_*.cbm.pkl"))
            if mdl is None:
...
# Load model: CatBoost (.cbm preferred; else pickled CatBoostClassifier)
